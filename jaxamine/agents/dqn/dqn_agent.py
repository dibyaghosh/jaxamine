...
from jaxamine.replay_memory import circular_replay_buffer
from flax import nn
...
class JaxDQNAgent(object):
  ...
  def __init__(self,
    ...
    Args:
      ...
      max_checkpoints_to_keep: int, the number of TensorFlow checkpoints to
        keep.
    ...
    logging.info('\t max_checkpoints_to_keep: %d',
  ...
  def set_eval_mode(self, eval_on):
    self.eval_mode = eval_on
    ...
    if self._replay.add_count > self.min_replay_history:
      if self.training_steps % self.update_period == 0:
        ...
        if (self.summary_writer is not None and
          ...
          self.summary_writer.add_scalar('HuberLoss', loss, self.training_steps)
